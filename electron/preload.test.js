...
describe('preload.js', () => {
  it('exposes electronAPI to the main world', () => {
    ...
    const preloadContent = fs.readFileSync(path.resolve(__dirname, 'preload.js'), 'utf-8');
    ...
    // Call back functions
    const cb1 = vi.fn(); api.onOpenFilePath(cb1);
    const cb2 = vi.fn(); api.onAppCloseRequested(cb2);
    const cb3 = vi.fn(); api.onMenuAction(cb3);
    const cb4 = vi.fn(); api.telemetry.onData(cb4);
    const cb5 = vi.fn(); api.telemetry.onStatus(cb5);
    const cb6 = vi.fn(); api.onUpdateAvailable(cb6);

    // Trigger callbacks
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'open-file-path')[1]({}, 'path');
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'app-close-requested')[1]({});
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'menu-action')[1]({}, 'action');
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'telemetry:data')[1]({}, 'data');
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'telemetry:status')[1]({}, 'status');
    mockIpcRenderer.on.mock.calls.find(call => call[0] === 'update-available')[1]({}, 'data');

...
async function run() {
  ...
  try {
    ...
    try {
      execSync(`npx jscpd src --reporters json --output tmp/jscpd --threshold 0 --min-tokens 40 --format typescript,javascript`, { stdio: 'pipe' });
    } catch (e) {
    ...
    if (duplicates.length === 0) {
      ...
      fs.writeFileSync(path.join(process.cwd(), 'DuplicationReport.txt'), emptyReport);

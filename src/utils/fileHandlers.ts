...
const electronAPI = (window as any).electronAPI as ExtendedElectronAPI | undefined;
...
function addToRecentFiles(path: string) {
    ...
    if (settings.recentFiles.length > 10) settings.recentFiles = settings.recentFiles.slice(0, 10);
        ...
        // Electron copy logic
         try {
            ...
            reader.onload = async (e) => {
                ...
                if (exists) {
                    if (!confirm(`File "${file.name}" already exists in the current directory. Overwrite?`)) {
                        loadProjectData(data);
                        return;
                    }
                }

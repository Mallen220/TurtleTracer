<script lang="ts">
  ...
  async function duplicateFile() {
    ...
    try {
      ...
      await electronAPI.writeFile(newFilePath, content);
      await loadDirectory();
  ...
  // NEW: Duplicate and mirror function
  async function duplicateAndMirrorFile() {
    ...
    try {
      ...
      const newFilePath = path.join(currentDirectory, newFileName);
      await electronAPI.writeFile(newFilePath, JSON.stringify(mirroredData));
      await loadDirectory();
      ...
      alert("Error duplicating and mirroring file: " + error.message);
    }
  }

  // NEW: Mirror path data function
  ...
  // NEW: Mirror point heading function
...
<div class="fixed inset-0 z-[1010] flex" class:pointer-events-none={!isOpen}>
  ...
  >
    <div class="flex flex-col h-full">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      <svg
                        ...
                        stroke-width={1.5}
                        stroke="currentColor"
                        class="size-4 text-neutral-500 dark:text-neutral-400 flex-shrink-0"
                      >
                        <path
                          stroke-linecap="round"
                          stroke-linejoin="round"
      ...
      {#if selectedFile}
        <div
          class="flex-shrink-0 p-4 border-t border-neutral-200 dark:border-neutral-700 space-y-2"
        >
          <div
            class="text-sm font-medium text-neutral-900 dark:text-white mb-2 truncate"
            title={selectedFile.name}
          >
            {selectedFile.name}
          </div>

          <!-- NEW: Duplicate and Mirror buttons -->
          <div class="flex gap-2">
            <button
              on:click={duplicateFile}
              class="flex-1 px-3 py-2 text-sm bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-colors flex items-center justify-center gap-2"
            ...
            <button
              ...
              class="flex-1 px-3 py-2 text-sm bg-purple-500 hover:bg-purple-600 text-white rounded-md transition-colors flex items-center justify-center gap-2"

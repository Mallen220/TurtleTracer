...
- **Hardcoded Paths**: The geometry is baked into the Java file. You do **not** need to upload the `.turt` file to the robot.
...
- **Dynamic Loading**: This mode uses `PedroPathReader` to load path data from the `.turt` file at runtime (legacy `.pp` supported). **You must upload your `.turt` file to the robot** for this to work.
...
- **Project File (.turt)**: Download the source file to share with teammates or back up your work (legacy `.pp` supported).

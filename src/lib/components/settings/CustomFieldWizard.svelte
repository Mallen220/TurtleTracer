...
<script lang="ts">
  ...
  function calculateConfig(): CustomFieldConfig | null {
      ...
      const distIn = Math.sqrt(Math.pow(w1.x - w2.x, 2) + Math.pow(w1.y - w2.y, 2));
  ...
  >
    ...
    >
      ...
      <div class="p-4 border-b border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
        <h2 class="text-xl font-bold text-neutral-900 dark:text-white">Custom Field Map Wizard</h2>
        <button on:click={handleClose} class="text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300">
      ...
      <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        <div class="flex items-center justify-center gap-2 mb-4">
            {#each [1, 2, 3, 4] as s}
                <div class={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step === s ? 'bg-blue-600 text-white' : step > s ? 'bg-green-500 text-white' : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-500'}`}>
                    {step > s ? '✓' : s}
                </div>
                {#if s < 4}
                    <div class={`w-8 h-0.5 ${step > s ? 'bg-green-500' : 'bg-neutral-200 dark:bg-neutral-800'}`}></div>
                {/if}
            {/each}
        </div>

        {#if step === 1}
            <div class="flex flex-col items-center justify-center h-64 border-2 border-dashed border-neutral-300 dark:border-neutral-700 rounded-lg bg-neutral-50 dark:bg-neutral-800/50">
                ...
                {#if imageData}
                    <button class="mt-4 text-blue-600 dark:text-blue-400 hover:underline" on:click={() => step = 2}>
        ...
        {:else}
            ...
            <div class="flex flex-col lg:flex-row gap-4 h-full min-h-[400px]">
                <div class="flex-1 relative bg-neutral-100 dark:bg-neutral-900 rounded-lg overflow-hidden flex items-center justify-center border border-neutral-200 dark:border-neutral-700">
                    <div class="relative max-w-full max-h-[60vh]" bind:this={imageContainer}>
                        ...
                        {#if p1}
                            <div class="absolute w-4 h-4 border-2 border-blue-500 rounded-full transform -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                                style={`left: ${(p1.x / imageElement.naturalWidth) * 100}%; top: ${(p1.y / imageElement.naturalHeight) * 100}%;`}>
                                <div class="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white text-xs px-1 rounded whitespace-nowrap">Point 1</div>
                            </div>
                        {/if}
                        {#if p2}
                            <div class="absolute w-4 h-4 border-2 border-red-500 rounded-full transform -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                                style={`left: ${(p2.x / imageElement.naturalWidth) * 100}%; top: ${(p2.y / imageElement.naturalHeight) * 100}%;`}>
                                <div class="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-red-500 text-white text-xs px-1 rounded whitespace-nowrap">Point 2</div>
                            </div>
                        {/if}
                    </div>
                </div>
                ...
                <div class="w-full lg:w-80 flex flex-col gap-4">
                    {#if step === 2}
                        <div class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                            <h3 class="font-bold text-blue-900 dark:text-blue-100 mb-2">Step 2: First Reference Point</h3>
                            <p class="text-sm text-blue-800 dark:text-blue-200 mb-4">Click a known point on the image (e.g., Top-Left corner of the field).</p>
                            ...
                            <div class="space-y-3">
                                ...
                                {:else}
                                    <div class="text-xs text-neutral-500 italic">Click on image to set point</div>
                                {/if}
                            </div>
                        </div>
                    {:else if step === 3}
                        <div class="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                            <h3 class="font-bold text-red-900 dark:text-red-100 mb-2">Step 3: Second Reference Point</h3>
                            <p class="text-sm text-red-800 dark:text-red-200 mb-4">Click another known point (e.g., Bottom-Right corner).</p>
                         ...
                         <div class="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                            ...
                            {#if calculateConfig()}
                                ...
                                <div class="space-y-2 text-sm">
                                    ...
                                    <div class="mt-4 p-2 bg-white dark:bg-neutral-800 rounded border border-neutral-200 dark:border-neutral-700">
                                        <p class="text-xs text-neutral-500 mb-1">Preview:</p>
                                        <div class="aspect-square w-full bg-neutral-100 relative overflow-hidden border border-neutral-300">
                                            <!-- Field bounds -->
                                            <div class="absolute inset-0 border-2 border-dashed border-neutral-400 z-10 pointer-events-none"></div>
                    ...
                    <div class="mt-auto flex justify-between gap-2 pt-4">
                        <button on:click={prevStep} disabled={step === 1} class="px-4 py-2 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded disabled:opacity-50">Back</button>
                        {#if step < 4}
                            <button on:click={nextStep} disabled={step === 2 && !p1 || step === 3 && !p2} class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                        {:else}
                            <button on:click={handleSave} class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Save & Apply</button>
                        {/if}
                    </div>
                </div>
            </div>

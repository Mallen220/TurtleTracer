...
<script lang="ts">
  ...
  $: isAlreadyRated = !!($settingsStore.submittedRatings && $settingsStore.submittedRatings[pkg.version]);
  ...
  async function submitFeedback() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        saveSettings($settingsStore).catch(e => console.error("Failed to save submitted ratings", e));
      ...
      <div class="px-6 py-5 space-y-6">
        <div>
          ...
          <div class="flex items-center gap-2">
            {#each [1, 2, 3, 4, 5] as star}
              ...
              >
                <svg
                  ...
                  stroke="currentColor"
                  stroke-width="1.5"
                  class="w-8 h-8 {rating >= star ? 'text-yellow-400' : 'text-neutral-300 dark:text-neutral-600'}"
                >
                  <path
            ...
            {/each}
            <span class="ml-2 text-sm font-medium {rating > 0 ? 'text-neutral-700 dark:text-neutral-300' : 'text-neutral-400'}">
        ...
        {#if status === "error"}
          <div class="p-3 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400 text-sm rounded-lg flex items-start gap-2 border border-red-200 dark:border-red-900/50">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 flex-shrink-0 mt-0.5">
              <path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0ZM8.94 6.94a.75.75 0 1 1-1.061-1.061 3 3 0 1 1 2.871 5.026v.361a.75.75 0 0 1-1.5 0v-.5c0-.83.67-1.5 1.5-1.5a1.5 1.5 0 1 0-1.415-2.262v-.001Zm-.75 6.559a1 1 0 1 0 2 0 1 1 0 0 0-2 0Z" clip-rule="evenodd" />
            </svg>
            <p>{errorMessage}</p>
          </div>
        {:else if status === "success"}
          <div class="p-3 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400 text-sm rounded-lg flex items-center gap-2 border border-green-200 dark:border-green-900/50">

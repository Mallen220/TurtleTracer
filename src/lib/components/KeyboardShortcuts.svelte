...
<script lang="ts">
  ...
  function removeSelected() {
    ...
    const sels = $multiSelectedPointIds.length > 0 ? $multiSelectedPointIds : ($selectedPointId ? [$selectedPointId] : []);
    ...
    // lines and sequence were already updated inside the stores by linesStore.update and sequenceStore.update calls above.
    // Re-fetching lines/sequence or relying on the reactive variables `lines` and `sequence` directly in .set()
    // without reactivity triggers is unsafe in Svelte 3 due to stale closures.
    // The previous update calls correctly mutated the stores.
  ...
  function LEGACY_movePoint(dx: number, dy: number) {
    if (isUIElementFocused()) return;

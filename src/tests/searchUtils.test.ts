...
describe('WhatsNew Search Utils', () => {
  ...
  describe('highlightSnippet', () => {
    ...
    it('should wrap match in span', () => {
      ...
      expect(result).toContain('hello <span class="bg-yellow-200 dark:bg-yellow-800 text-neutral-900 dark:text-neutral-100 rounded-sm px-0.5">world</span>');
    });
  ...
  describe('highlightText (DOM)', () => {
      ...
      beforeEach(() => {
          dom = new JSDOM('<!DOCTYPE html><div id="root">Hello World. This is a test.</div>');
      ...
      it('should highlight text in DOM', () => {
          ...
          expect(root.innerHTML).toContain('<mark class="bg-yellow-200 dark:bg-yellow-800 text-neutral-900 dark:text-neutral-100 rounded-sm px-0.5">test</mark>');
      });

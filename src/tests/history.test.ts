...
describe('createHistory', () => {
  ...
  it('respects max size', () => {
    ...
    // 5 -> 4
    expect(history.undo()).toEqual({ ...defaultState, startPoint: { x: 4, y: 0 } });
    // 4 -> 3
    expect(history.undo()).toEqual({ ...defaultState, startPoint: { x: 3, y: 0 } });
    // 3 -> 2
    expect(history.undo()).toEqual({ ...defaultState, startPoint: { x: 2, y: 0 } });
    // 2 -> 1
    expect(history.undo()).toEqual({ ...defaultState, startPoint: { x: 1, y: 0 } });

    // Now at state 1. Stack has [state1]. length is 1. canUndo should be false.
    expect(history.canUndo()).toBe(false);
    expect(history.peek()).toEqual({ ...defaultState, startPoint: { x: 1, y: 0 } });
  });
